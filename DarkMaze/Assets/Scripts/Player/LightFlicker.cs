...
{
    // This name 'LightFlickerPulse' is a very convenient name, trust me!
    ...
    public enum UseType
    {
        ...
        Range,
        Off
    } // The way you want to affect the light in a nice drop down menu!
    ...
    public enum WaveFunction
    {
        ...
        Saw,
        SawInv
    } // The type of function you want to use in a drop down menu!
    ...
    public float phase;
    // Acts like a seed and changes the lifetime of the script, doesn't do much unless you want to be specific!
    ...
    // Hidden variables to remember the single light(s) colours on different gameObjects

    private float[] mainOutValueI = new float[49];
    // Hidden variables to remember the single light(s) intensities on different gameObjects

    private float[] mainOutValueR = new float[49];
    // Hidden variables to remember the single light(s) ranges on different gameObjects
    ...
    // Hidden variable to make sure you have assigned everything, almost an on/off switch to control if it works or not!
    ...
    private void Update()
    {
        ...
        if (!assigned)
        {
            if (GetComponent<Light>() != null)
            {
                ...
                //Debug.Log("mainValueI: " + mainValueI);
                if (error1 && displayScriptErrors)
                {
                    Debug.Log(
                        "FIXED ERROR REPORT: The missing Light component on the gameObject " + "'" + gameObject.name +
                        "'" + " has been successfully re-established!", gameObject);
                    error1 = false;
                }
                assigned = true;
            }
            ...
            {
                Debug.LogError(
                    "ERROR MISSING COMPONENT: The gameObject " + "'" + gameObject.name + "'" +
                    " does not have a Light component attached to it. If you wish to use a Light from another gameObject, please select the boolean variable 'Use Outside Light' for this feature.",
                    gameObject);
                error1 = true;
            }
            if (outsideLight.Length > 0 && outsideLight.Length < 50 && useOutsideLight)
            {
                for (int i = 0; i < outsideLight.Length; i++)
                {
                    if (outsideLight[i] == null && displayScriptErrors && !error2)
                    {
                        error2 = true;
                        Debug.LogError(
                            "ERROR EMPTY VARIABLE: The gameObject " + "'" + gameObject.name + "'" +
                            " does not have a gameObject with a Light component attached to the variable 'Outside Light' index number " +
                            "[ " + i +
                            " ], to fix this either, assign a gameObject with a Light component in this space or, lower the size amount to fit your assigned lights. If you do not wish to use this feature, un-tick the boolean variable 'Use Outside Light.'",
                            gameObject);
                        Debug.LogError(
                            "VARIABLE ADDITIONAL INFORMATION: This feature does not allow for fixing after the error has occurred due to strict limitations of the 'for' loop, you must replay with the Light component attached to the variable 'Outside Light' index number " +
                            "[ " + i + " ], correctly.", gameObject);
                    }
                    else if (!error2)
                    {
            ...
            {
                Debug.LogError(
                    "ERROR EXCEEDING ARRAY LIMITATIONS: Due to this script having to pre-load other array variables to gather all the Light components information a decision was made on how many arrays as max should be pre-defined to remove any performance issues. To fix this error you could modify this script to allocate more available spots located in the defining variables section or remove some or many Light components from the array list on the gameObject " +
                    "'" + gameObject.name +
                    "'. The defined Light components past the 48th index number will not work under this script.",
                    gameObject);
            }
        }
        if (((!error1 && !useOutsideLight) || (!error2 && useOutsideLight)) && assigned && useType != UseType.Off &&
            ...
            else
            {
                ...
                switch (useType)
                {
                    case UseType.Color:
                        ...
                        break;
                    case UseType.Intensity:
                        //Debug.Log("lightS.intensity: " + lightS.intensity);
